...
class Figura(Kwadrat): # dziedziczymy po klasie Kwadrat aby móć skorzystać z jej funkcjonalności
    def sketchFigura(self, x, y, paski): # dodajemy ilość pasków, w które ma być kwadrat
        Figura.sketch(self, x, y) # korzystamy z metody klasy bazowej
        space = self.bok/paski # wyliczamy przerwęmiędzy paskami
        _xLinii_ = 0 # to jest pole chronione, nie powinno się go używać w kodzie po za klaą i klasami po niej dziedziczącymi 
        for pasek in range(0, paski): # dorysowujemy paski
            line(x+_xLinii_, y, x+_xLinii_, y+self.bok)
...
def setup():
    ...
    fill(255, 100, 60, 150)
    duzaFigura = Figura(120.0) 
    duzaFigura.sketchFigura(25, 335, 10)
    malaFigura = Figura(50.0)
    malaFigura.sketchFigura(175,425, 3)
